...
static int g_core_has_singstar = -1; /* -1=unknown 0=no 1=yes */
static int g_core_has_player1_key = -1;
static int g_core_has_usb_mic = -1;
...
static void run_startup_sequence(void) {
   ...
   * Brute-force every key layout seen across PCSX2 / Aether generations. */
  ...
  prefs_set_string("USB1/microphone_subtype", "0");
  ...
  prefs_set_string("USB1/player1_device_name", "switch_usb_mic_0");
  prefs_set_string("USB1/player2_device_name", "switch_usb_mic_1");
  prefs_set_string("USB1/input_device_name", "switch_usb_mic_0");
  prefs_set_string("USB1/microphone_player1_device_name", "switch_usb_mic_0");
  prefs_set_string("USB1/microphone_player2_device_name", "switch_usb_mic_1");
  prefs_set_string("USB1/microphone_input_device_name", "switch_usb_mic_0");
  /* Also try Type name "microphone" (newer PCSX2 Name vs TypeName confusion) */
  prefs_set_string("USB/Port1/Type", "singstar");
  prefs_set_string("USB/Port1/Device", "singstar");
...
/* Scan loaded libemucore for USB-mic related strings. */
static int so_mem_contains(const char *needle)
{
  if (!emu_mod.load_virtbase || !emu_mod.load_size || !needle || !needle[0])
    return 0;
  const size_t nlen = strlen(needle);
  if (nlen == 0 || nlen > emu_mod.load_size) return 0;
  const char *base = (const char *)emu_mod.load_virtbase;
  const size_t limit = emu_mod.load_size - nlen;
...
static void scan_core_usb_strings(void)
{
  g_core_has_singstar = so_mem_contains("singstar");
  g_core_has_player1_key = so_mem_contains("player1_device_name");
  g_core_has_usb_mic = so_mem_contains("USB-Mic") || so_mem_contains("USBMic") ||
                       so_mem_contains("MicrophoneDevice");
}
